...
setwd("C:/Users/alex-/Desktop/Tesis/Articulos de investigación/Memorias de congreso_2023/TODO")
...
df21=pdf_text("Contemporáneo.pdf")#
df22=pdf_text("Genealógica.pdf")
df23=pdf_text("TECNOLOGIAE.pdf")#
...
FrecuenciaMC<-bind_rows(
  ...
  mutate(df15,line= row_number(), `Plan`="Pedagogíco"),
  ...
  mutate(df21,line= row_number(), `Plan`="Contemporáneo"),
  mutate(df22,line= row_number(), `Plan`="Genealógica"),
  mutate(df23,line= row_number(), `Plan`="TECNOLOGIAE"),
  mutate(df24,line= row_number(), `Plan`="Pedagógica"),
  mutate(df25,line= row_number(), `Plan`="Pragmática"),
  mutate(df11,line= row_number(), `Plan`="TutoríaEN")) %>% 
  count(Plan, word) %>%
  group_by(Plan) %>%
...
FrecuenciaMC=na.omit(FrecuenciaMC)
FrecuenciaMC
...
setwd("C:/Users/alex-/Desktop/Tesis/Articulos de investigación/Revistas/TODOS")
...
df1=pdf_text("Agua.pdf")
df2=pdf_text("Ansiedad.pdf")
df3=pdf_text("ConsumoAlcohol.pdf")
df4=pdf_text("Plantas.pdf")
df5=pdf_text("Riqueza.pdf")
df6=pdf_text("Education.pdf")
df7=pdf_text("Factors.pdf")#
df8=pdf_text("Gameplay.pdf")
df9=pdf_text("Infres Oaxaca.pdf")#
df10=pdf_text("Interpreting.pdf")
df11=pdf_text("Literature review.pdf")
df12=pdf_text("Networking.pdf")
df13=pdf_text("Neural machine.pdf")
df14=pdf_text("Podcasts.pdf")
df15=pdf_text("Robots.pdf")
df16=pdf_text("Smartwatches.pdf")
df17=pdf_text("Systematic review.pdf")
df18=pdf_text("Technology.pdf")
df19=pdf_text("Virtual reality.pdf")
df20=pdf_text("Website.pdf")
df21=pdf_text("Pacientes.pdf")
df22=pdf_text("Osteoartrite.pdf")
df23=pdf_text("Maxilofaciais.pdf")
df24=pdf_text("Instablity.pdf")
df25=pdf_text("Artropatia.pdf")
...
FrecuenciaAR<-bind_rows(
  mutate(df1,line= row_number(), `Plan`="Agua"),
  mutate(df2,line= row_number(), `Plan`="Ansiedad"),
  mutate(df3,line= row_number(), `Plan`="ConsumoAlcohol"),
  mutate(df4,line= row_number(), `Plan`="Plantas"),
  mutate(df5,line= row_number(), `Plan`="Riqueza"),
  mutate(df6,line= row_number(), `Plan`="Education"),
  mutate(df7,line= row_number(), `Plan`="Factors"),
  mutate(df8,line= row_number(), `Plan`="Gameplay"),
  mutate(df9,line= row_number(), `Plan`="Infres Oaxaca"),
  mutate(df10,line= row_number(), `Plan`="Interpreting"),
  mutate(df11,line= row_number(), `Plan`="Literature review"),
  mutate(df12,line= row_number(), `Plan`="Networking"),
  mutate(df13,line= row_number(), `Plan`="Neural machine"),
  mutate(df14,line= row_number(), `Plan`="Podcasts"),
  mutate(df15,line= row_number(), `Plan`="Robots"),
  mutate(df16,line= row_number(), `Plan`="Smartwatches"),
  mutate(df17,line= row_number(), `Plan`="Systematic review"),
  mutate(df18,line= row_number(), `Plan`="Technology"),
  mutate(df19,line= row_number(), `Plan`="Virtual reality"),
  mutate(df21,line= row_number(), `Plan`="Pacientes"),
  mutate(df22,line= row_number(), `Plan`="Osteoartrite"),
  mutate(df23,line= row_number(), `Plan`="Maxilofaciais"),
  mutate(df24,line= row_number(), `Plan`="Instablity"),
  mutate(df25,line= row_number(), `Plan`="Artropatia"),
  mutate(df20,line= row_number(), `Plan`="Website")) %>% 
  count(Plan, word) %>%
  group_by(Plan) %>%
  mutate(proportion = n / sum(n)) %>% 
  #dplyr::select(-n) %>% # Para eliminar el problema con el select de la libería Mass
  spread(Plan, proportion) %>% 
  gather(Plan, proportion, `Agua`:`Website`)

FrecuenciaAR=na.omit(FrecuenciaAR)
FrecuenciaAR
...
Frecuencia1=rbind(FrecuenciaMC,FrecuenciaAR)
...
Frecuencia1 %>% 
  ...
  geom_col() + 
  theme_bw()+
  geom_text(aes(label=conteo), size=4.5, color="black", vjust = 0.5)+
  labs(title = "Conteo de Palabras",
       subtitle = "En Memorias de congreso y Articulos de revistas de investigacion en español, ingles y portuguese",
...
colnames(word_freq_df1)=c("Word","Frequency","Documentos")

x11()
ggplot(word_freq_df1, aes(x = rank, y = Frequency, color = Documentos)) +
  geom_line() +
  ...
  labs(title = "Ley de Zipf con Conteo de Palabras",
       subtitle = "En Memorias de congreso y Articulos de revistas de investigación en español, ingles y portuguese",
       caption = "Fuente: Elaboración propia")+
  theme(plot.title=element_text(hjust=0.5),
        plot.subtitle=element_text(hjust=0.5),
        plot.caption = element_text(hjust = -0.03))
