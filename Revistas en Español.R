...
Frecuencia<-bind_rows(
  ...
  gather(Plan, proportion, `Agua`:`Riqueza`)
...
Frecuencia1 %>% 
  ...
  labs(title = "Conteo de Palabras",
       subtitle = "Revistas de Investigación",
...
colnames(word_freq_df1)=c("Word","Frequency","ARIE")

x11()
ggplot(word_freq_df1, aes(x = rank, y = Frequency, color = ARIE)) +
  geom_line() +
  ...
  labs(title = "Ley de Zipf con Conteo de Palabras",
       subtitle = "Revistas de investigación en español",
       caption = "Fuente: Elaboración propia")+
  theme(plot.title=element_text(hjust=0.5),
        plot.subtitle=element_text(hjust=0.5),
        plot.caption = element_text(hjust = -0.03))
